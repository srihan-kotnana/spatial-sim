...
class POI(Place):
    ...
    def get_agent_dwell_time(self, use_data=True): # returns a likely dwell time from the POI's dwell dist
        if use_data:
            return self.dwell_dist[np.random.randint(0, len(self.dwell_dist))]
        else:
            return np.random.randint(0, 95) # random up to the length of the day
